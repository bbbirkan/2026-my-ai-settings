...
async def _stream_response(task: str, cid: str, loop_answer: str = None) -> AsyncGenerator[str, None]:
    """OpenAI SSE akışı — orchestration yaparken keepalive gönderir."""
    if loop_answer is not None:
        answer = loop_answer
    else:
        # Keepalive gönderirken orchestrate çalıştır
        result_holder: dict = {}
        async def run_orch():
            result_holder["result"] = await orchestrate(task, mode="parallel", wiki=True)
...
async def chat_completions(req: ChatCompletionRequest):
    ...
    if assistant_messages:
        answer = assistant_messages[-1].content or ""
        if req.stream:
