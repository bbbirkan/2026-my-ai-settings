...
### ⚠️ Dual-Model Confusion Pitfall

Users frequently panic when they see a model name in `config.yaml` that doesn't match what they think they're using. Example: "Config'te `gemini-2.0-flash` var ama Telegram'da `kimi-k2.6` çalışıyor, neden?" This is NOT a bug.

**Why it happens:** `model.default` in config.yaml is the **gateway internal model** — used for sub-agents, cron jobs, autonomous tasks. The messaging platform (Telegram, Discord, etc.) has its OWN model selection, which may differ.

| Layer | Model | Used for |
|------|-------|----------|
| Messaging platform (Telegram/Discord) | Whatever is set in the platform config | Conversational turns with user |
| Gateway internal | `model.default` in config.yaml | Sub-agents, cron, delegation |

**When to change which:**
- Change `model.default` (config.yaml) → affects sub-agents, cron, autonomous work
- Change platform model → affects only that platform's conversational turns

**Pitfall — DO NOT:**
- Tell the user their "real model" changed when you only changed `model.default` — they're using two models
- Let the user think they're paying for two model runs per message — they're not
- Change `model.default` without explaining it only affects background tasks

**Right explanation:** "Gateway internal model değişti, Telegram konuşması farklı model kullanıyor. İki ayrı katman."
...
❌ API failed after 3 retries — HTTP 404: No endpoints found that support tool use.
```

**What it means:** The model currently in use does not support tool calling (function calling) on OpenRouter. Hermes Agent requires tool calling to operate — without it, every API call fails.

**Common causes:**
- Manual model switch (e.g. `/model deepseek/deepseek-v4-pro`) to a model without tool use
- Uncensored-model fallback that selected a small model (8B–12B) without tool use endpoints
- AGENTS.md or skill fallback lists referencing models that don't support tools on OpenRouter
...
1. Check the active model:
   ...
   ```
2. Check gateway logs for the error context:
   ```bash
   grep -i "retry\|404\|tool use\|fallback" ~/.hermes/logs/gateway.log | tail -20
   ```
3. Search recent sessions for the error:
   Use session_search with query "tool use" or "API failed"
4. Verify the model supports tool use — OpenRouter models page or the model's card shows this.

**Pitfall — DO NOT:**
- Change the user's model without asking first. Even if you know the fix, *propose* it, don't apply it.
- Switch to an expensive model (Claude, GPT-4o) as a knee-jerk fix when the user has cheaper alternatives configured.
- Modify AGENTS.md or skill files without explicit permission — especially fallback lists the user curated.

**Fix workflow:**
1. Diagnose (steps above)
2. Tell the user: *"Model X lacks tool use. Options: switch to Y ($price) or Z ($price). Which?"*
3. Only apply the change after confirmation
4. Remind user: `/reset` needed for new model to take effect
